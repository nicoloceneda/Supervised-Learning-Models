...
def plot_decision_regions(X, y, classifier, resolution=0.02, test_idx=None):
    ...
    """ Create a colormap object.
        ...
        features. The matrix with two columns is needed because the support vector machine was trained on a matrix with such shape.

        Use the predict method of the svm to predict the class corresponding to all the possible combinations of features generated in the
        ...
        training phase, we expect the svm to find a decision boundary that correctly classifies all the samples in the training and test sets.
        ...
        To verify that the svm correctly classified all the samples in the training set, plot the the original features in the scatter plot
    ...
    if test_idx:
        ...
        plt.scatter(X_test[:, 0], X_test[:, 1], alpha=0.8, linewidth=1, color='none', marker='s', edgecolor='black', label='test_set')
...
plt.savefig('images/04_support_vector_machine_gd_sl/Decision_boundary_and_training_sample.png')
